...
/// </summary>
public class MCRcon : Queue<MCRconPacket>
{
    ...
    Thread? bgCommThread;
    Queue<MCRconPacket> cmdQue = new Queue<MCRconPacket>();
    ...
    public MCRcon(string host, int port, string password)
        : base()
    {
        RConHost = host;
        RConPort = port;
    ...
    {
        cmdQue.Enqueue(MCRconPacket.CmdPacket(Command, sessionID));
    }
    ...
    /// </summary>
    public bool IsStarted { get { return bgCommThread != null && bgCommThread.IsAlive; } }
    /// <summary>
    ...
    private void ConnectAndProcess()
    {
        ...
        using (TcpClient cli = new TcpClient())
        {
            ...
            try
            {
                ...
                MCRconPacket auth = MCRconPacket.AuthPacket(RConPass, sessionID);
                auth.SendToNetworkStream(cli.GetStream());

                if (auth.IsBadPacket == false)
                {
                    MCRconPacket resp = new MCRconPacket();
    ...
    private void Comms(TcpClient cli)
    {
        ...
        try
        {
            ...
            while (AbortTCP == false)
            {
                ...
                if (cmdQue.Count > 0)
                {
                    MCRconPacket Cmd = cmdQue.Dequeue();
...
public class MCRconAsync : Queue<MCRconPacket>
{
    public enum TCPState { IDLE, CONNECTING, CONNECTED, CLOSING, CLOSED, ABORTED };
    public enum RConState { IDLE, AUTHENTICATE, READY, NETWORK_FAIL, AUTHENTICATE_FAIL };
    public string? LastTCPError { get; set; }
    ...
    /// <param name="port">Port RCon is listening to on the server.</param>
    /// <param name="password">Configured password for the RCon server.</param>
    public MCRconAsync(string host, int port, string password)
        : base()
    {
        RConHost = host;
        RConPort = port;
    ...
    private async Task CommsAsync(TcpClient cli)
    {
        ...
        async Task<MCRconPacket> FetchNextPacket()
        {
            MCRconPacket resp = new MCRconPacket();
            await resp.ReadFromNetworkSteamAsync(cli.GetStream());
            return resp;
        ...
        try
        {
            ...
            while (AbortTCP == false)
            {
                ...
                if (cli.Available > 0)
                {
                    MCRconPacket resp = await FetchNextPacket();
