...
{
    public static async Task Parse(DiscordRuntime runtime, RequestBlocks_Block block)
    {
        if (block.inputs.TryGetValue("id", out RequestBlocksBlock? idBlock) && idBlock.block != null && idBlock.block.enabled)
        {
            switch (block.type)
            {
                case "obj_component_modal_input_text":
                    ...
                    if (Match != null && block.inputs.TryGetValue("output_text", out RequestBlocksBlock? textBlock) && textBlock.block != null)
