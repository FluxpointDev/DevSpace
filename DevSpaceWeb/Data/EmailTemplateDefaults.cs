...
public static class EmailTemplateDefaults
{
    ...
    {
        { EmailTemplateType.Header, "{instance.icon} {.format-center}\n{instance.name} {.format-center}" },
        { EmailTemplateType.Footer, "Need support?\nContact us at {instance.email}" },
        { EmailTemplateType.Test, "This is a test message from {instance.name}." },
        { EmailTemplateType.AccountInvited, "You have been invited to create an account on {instance.name}\n\nCreate one here {instance.url}" },
        { EmailTemplateType.AccountJoinTeam, "You have been invited to join the team {team.name} on {instance.name}\n\nClick here to join {action}" },
        { EmailTemplateType.AccountConfirm, "Click here to confirm your account {user.name}\n\n[Confirm Account]({action})" },
        ...
        { EmailTemplateType.Account2FADisabled, "2FA (Two-factor) authentication for your account {user.name} has been removed!" },

        { EmailTemplateType.AccountEnabled, "Your account {user.name} has been activated on {instance.name}" },
        { EmailTemplateType.AccountDisabled, "Your account {user.name} has been disabled on {instance.name}" },

        { EmailTemplateType.AccessCode, "You have requested an email code for your account {user.name}\nThis was sent due to {purpose}.\n\nUse the code **{code}** to confirm." },
